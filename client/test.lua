...
RegisterNetEvent('qb-menu:client:secMenu', function(data) --Secondary Police Menu
    exports['qb-menu']:openMenu({
        ...
        {
            header = 'Take DNA Sample',
            txt = 'Take a DNA sample from a player',
            icon = 'fas fa-code-merge',
            params = {
                event = 'qb-menu:client:dnaMenu',
            }
        },
        {
            header = 'Next',
            ...
            icon = 'fas fa-code-pull-request',
            params = {
                event = 'qb-menu:client:paymentMenu',
            }
        },
        {
            header = 'Back',
...
RegisterNetEvent('qb-menu:client:dnaMenu', function(data) --DNA Menu
    exports['qb-menu']:openMenu({
        {
            header = 'DNA',
            icon = 'fa-solid fa-backward',
        ...
        },
        {
            ...
            icon = 'fas fa-code-merge',
            params = {
                event = 'police:server:TakeDNA',
            },
            dialog = function()
                local setheader = "Take DNA Sample"
                local setinputs = {
                    {
                ...
                local dialog = exports['qb-input']:ShowInput({ header = setheader, submitText = "Submit", inputs = setinputs })
                return dialog
            end,
            event = function(data)
                if data.PlayerId then
                    local src = source
                    -- Hash the Player ID for DNA
                    local dnaHash = DnaHash(data.PlayerId)
                    -- Trigger the server event with the hashed DNA
                    TriggerServerEvent('police:server:TakeDNA', dnaHash)
                else
...
RegisterNetEvent('qb-menu:client:paymentMenu', function(data)
    exports['qb-menu']:openMenu({
        {
        ...
        {
            header = 'Pay Lawyer',
            txt = 'Pay a lawyer for their services',
            icon = 'fas fa-code-merge',
            dialog = function()
                local setheader = "Pay Lawyer"
                local setinputs = {
                    {
                        text = "Enter Player ID",
                        name = "PlayerID",
                ...
                return exports['qb-input']:ShowInput({ header = setheader, submitText = "Submit", inputs = setinputs })
            end,
            event = function(data)
                if data.PlayerID then
                    TriggerServerEvent('police:server:PayLawyer', data.PlayerID)
                else
                    QBCore.Functions.Notify("Player ID is missing!", "error")
                end
            end
        }
    })
